...
static int add_merge( struct st **ptr, char *start_time, char *stop_time) 
{
...
static int get_time_select(int n_scheduleid )
{
	...
	char stop_time[8][8] = {"9:50","6:40","14:00","9:00","3:00","10:30","23:52" };
        ...
        char week[8][8] = {"SUN","MON","TUE","WED","THU","FRI","SAT"};
	struct st *tmp,*tmp1;
	struct st *weekday[8] = { NULL };
        for (i=0; i < n_scheduleid; i++) {
                                        days = 7;
                                        if (days < 8) {
						add_merge(&weekday[days - 1 ], start_time[j], stop_time[j]);
					//	update_struct_tbs_day(&weekday[days - 1 ], start_time[j], stop_time[j]);
                                                ...
                                                for ( ; k < days-2; k++) {
							add_merge(&weekday[k ], start_time[j], stop_time[j]);
...
/*	strcpy(weekday[0]->stop_time,"NIKHIL");
	if(weekday[0]->stop_time[0] == '\0') {
		printf("TEST \n");
	} else {
		printf("TEST1%s2 %d\n",weekday[0]->stop_time,weekday[0]->enable);
